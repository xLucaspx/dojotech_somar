...
function criaCardProjeto(dados) {
	...
	const conteudo = `
    <a href="projeto.html?id=${dados.id}" title="Abrir página do projeto ${
		dados.name
	}" class="projeto__link">
...
function criaMiniaturaMidia(dados) {
	...
	if (fileTypes.video.includes(dados.type)) {
		conteudo = `
...
function criaItemOds(ods) {
	const li = document.createElement("li");
	li.innerHTML = `<img src="${BASE_URL + ods.imageUrl}" alt="${ods.id} - ${ods.name}" class="projeto__ods__img midia">`;
